...
SELECT t.НаимТипаСырья, p.НаимСырья, ROUND(s.Цена, 2), s.Количество, CONVERT(varchar, s.Датадвижения, 101) 'дата'
FROM Склад AS s
...
SELECT s.КодСкладаДвиж, DATEPART(week, s.Датадвижения) AS 'неделя',
  COUNT(p.КодСырья) AS 'кол-во'
FROM Склад AS s
  INNER JOIN Сырье AS p ON s.КодСырья = p.КодСырья
WHERE p.КодТипаСырья = 9
  AND YEAR(s.Датадвижения) = 2003
  AND DATEPART(week, s.Датадвижения) % 2 = 0
  AND s.ПризнакДвижения = 'Поступление'
GROUP BY s.КодСкладаДвиж, DATEPART(week, s.Датадвижения)
ORDER BY s.КодСкладаДвиж, 'неделя';
...
SELECT n.НаимСклада, DATEPART(week, s.Датадвижения) AS 'неделя',
  COUNT(DISTINCT p.КодСырья) AS 'кол-во'
FROM Склад AS s
  INNER JOIN Сырье AS p ON s.КодСырья = p.КодСырья
  INNER JOIN Список_складов n on s.КодСклада = n.КодСклада
WHERE p.КодТипаСырья = 9
  AND YEAR(s.Датадвижения) = 2003
  AND DATEPART(week, s.Датадвижения) % 2 = 0
  AND s.ПризнакДвижения = 'Поступление'
