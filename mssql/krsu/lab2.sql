...
INSERT INTO Страны ([Наименование страны])
VALUES ('Россия'), ('Кыргызстан'), ('Казахстан');

INSERT INTO Города ([Наименование города], [Код страны])
VALUES ('Бишкек', 2), ('Ош', 2),
  ('Москва', 1), ('Омск', 1), ('Томск', 1),
  ('Астана', 3), ('Шымкент', 3), ('Алмата', 3);

INSERT INTO Сотрудники (Отдел, Фамилия, Имя, Отчество,
...
ON Сотрудники (Фамилия, Имя, Отчество, [Дата рождения]);
...
ALTER TABLE Сотрудники
  DROP CONSTRAINT FK_Сотрудники_Города;
-- Теперь можно.
TRUNCATE TABLE Сотрудники;
-- Возвращаю ограничение.
ALTER TABLE Сотрудники
  ADD CONSTRAINT FK_Сотрудники_Города FOREIGN KEY ([Код города])
    REFERENCES Города ([Код города])
    ON DELETE CASCADE
    ON UPDATE CASCADE;
